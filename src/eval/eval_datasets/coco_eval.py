...
from src.eval.eval_datasets.coco_eval_transforms import COCOEvalTransforms, COCOEvalCollator
# from coco_eval_transforms import COCOEvalTransforms, COCOEvalCollator
...
class COCOEval(Dataset):
    ...
    def __init__(
        ...
        self.load_sem_mask = True
        self.use_boxes = use_boxes
        

    def bbox_to_mask(self, boxes, image):
        N = len(boxes)
        h, w  = image.shape[:2]
        masks = np.zeros((h, w))
        for i in range(0, N):
            x_min, y_min = int(round(boxes[i, 0].item())), int(round(boxes[i, 1].item()))
            x_max, y_max = int(round(boxes[i, 2].item())), int(round(boxes[i, 3].item()))
    ...
    def __getitem__(self, idx):
        ...
        if self.use_boxes:
            box_mask = self.bbox_to_mask(annos, image)
        else:
    ...
    def load_annos(self, idx):
        ...
        for anno in annos:
            ...
            target[0, 4] = coco_lbl + 1 # to put background as 0
        ...
        return targets.astype(np.float32) # [N, 5 (x1, y1, x2, y2, cat_id)], [N, 512]
...
def build_coco_eval_dataset(params, val_only=False, tta=False, suppress_idx=False):
    ...
    val_transforms = COCOEvalTransforms(
        resolution,
        norm_mean=norm_mean,
        norm_std=norm_std,
        val=True,
    ...
    args = dict(
        data_root=params.dataset_root,
        coco_transforms=val_transforms,
        split='val',
        load_anno=True,
    )
    val_dataset = COCOEval(**args)
    if val_only:
        return val_dataset, COCOEvalCollator()
    args['split'] = 'train'
    args['load_anno'] = True 
    args['coco_transforms'] = COCOEvalTransforms(
        resolution,
        norm_mean=norm_mean,
        norm_std=norm_std,
        val=False,
    )
    train_dataset = COCOEval(**args)
    if not tta:
        return train_dataset, val_dataset, COCOEvalCollator(suppress_idx=suppress_idx)
    else:
        # Test time adaption by training on the val dataset
        return val_dataset, val_dataset, COCOEvalCollator(suppress_idx=suppress_idx)

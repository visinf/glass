...
class VisualizationMethod(ABC):
    ...
    @abstractmethod
    def __call__(self, *args, **kwargs) -> Visualization:
...
class Image(VisualizationMethod):
    ...
    def __call__(
        ...
        if self.as_grid:
            return Image(make_grid(image, nrow=self.n_row))
        else:
            return Images(image)
...
class Video(VisualizationMethod):
    ...
    def __call__(self, video: torch.Tensor) -> Video:
        """Visualize video.
        ...
        return Video(video, fps=self.fps)
...
class Mask(VisualizationMethod):
    ...
    def __call__(
        ...
        if masks.dim() == 5:
            ...
            return Video(mask_vis, fps=self.fps)
        elif masks.dim() == 4:
            # Handling image data.
            ...
            # Draw masks inverted as they are easier to print.
            return Image(make_grid(1.0 - masks, nrow=n_objects))
        else:
...
class VisualObject(VisualizationMethod):
    ...
    def __call__(
    ...
    ) -> Union[Dict[str, Image], Dict[str, Video]]:
        ...
        if objects.dim() == 6:
            ...
            mask_vis = mask_vis.unsqueeze(0)
            return {
                "reconstruction": Video(object_vis, fps=self.fps),
                "mask": Video(mask_vis, fps=self.fps),
            }
        elif objects.dim() == 5:
            # Handling image data.
            ...
            return {
                "reconstruction": Image(object_grid),
                # Draw masks inverted as they are easier to print.
                "mask": Image(make_grid(1.0 - masks, nrow=n_objects)),
            }
        else:
            raise RuntimeError("Unsupported tensor dimensions.")
            ...
            masks_on_image = torch.stack(
                [
                    draw_segmentation_masks(
                        (255 * img).to(torch.uint8), mask, alpha=self.alpha, colors=cmap
                    )
                    for img, mask in zip(input_image.to("cpu"), masks_one_hot.to("cpu"))
                ]
            )

            return Image(make_grid(masks_on_image, nrow=8))
        return None
...
class Flow(VisualizationMethod):
    ...
    def __call__(self, flow: torch.Tensor):
        ...
        if self.as_grid:
            return Image(make_grid(flow, nrow=self.n_row))
        else:
            return Images(flow)
...
class TrackedObject_from_Mask(VisualizationMethod):
    ...
    def __call__(
        ...
        return Video(rendered_video, fps=self.fps)

...
def masks_to_boxes(masks, num_slots=7):
    """Convert seg_masks to bboxes.

    Args:
        masks: [B, H, W], output after taking argmax
    ...
    """
    B, H, W = masks.shape
    binary_masks = F.one_hot(masks, num_slots)  # [B, H, W, N]
    binary_masks = binary_masks.permute(0, 3, 1, 2)  # [B, N, H, W]
    binary_masks = binary_masks.contiguous().flatten(0, 1)
    bboxes = masks_to_boxes_w_empty_mask(binary_masks)  # [B*N, 4]
    bboxes = bboxes.reshape(B, -1, 4)  # [B, N, 4]
    return bboxes
...
def acc_metric(pred_labels, gt_labels):
    pred_labels = pred_labels.argmax(-1)
    # gt_labels = [d.tolist() for d in gt_labels]
    bs = len(gt_labels)
    acc = 0
    for idx, items in enumerate(gt_labels):
        acc = acc + torch.sum(pred_labels[idx][:len(items)]==items) / len(items)
...
def ari_precision_recall(segmentation_gt: torch.Tensor, segmentation_pred: torch.Tensor,
  mode: str, adjusted: bool):
  ...
  oh_segmentation_gt = F.one_hot(segmentation_gt, max_classes).float()
  oh_segmentation_pred = F.one_hot(segmentation_pred, max_classes).float()

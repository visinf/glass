...
class MultiHeadSTEVESA(ModelMixin, ConfigMixin):
    ...
    def __init__(self, num_iterations, num_slots, num_heads,
        ...
        if init_mlp:
            self.bbox_mlp = nn.Sequential(nn.Linear(4, slot_size*2), 
                                          nn.ReLU(),
                                          nn.Linear(slot_size*2, slot_size))
        
    def forward(self, inputs, num_objs=[], use_mask=False, bbox=None):
        slots_collect, attns_collect = self.forward_slots(inputs, num_objs=num_objs, use_mask=use_mask, bbox=bbox)
    ...
    def forward_slots(self, inputs, num_objs=[], use_mask=False, bbox=None):
        """
        ...
        if self.learnable_slot_init:
            ...
            slots = self.bbox_mlp(bbox[:, :self.num_slots, :4])
        else:
            # initialize slots
        ...
        for t in range(T):
            ...
            for i in range(self.num_iterations):
                ...
                if use_mask:
                    # print('Using dyanuc')
                    indices = torch.arange(self.num_slots, device=num_objs.device)
                    masks = indices.unsqueeze(0).expand(B, -1) >= num_objs.unsqueeze(1)
                    masks = masks.to(attn_logits.device)
                    attn_logits.masked_fill_(masks.to(torch.bool).view(B, 1, 1, self.num_slots), float("-inf"))
